...
def main() -> None:
    ...
    story += [
        p(
            "在val.csv上，使用清洗训练集的最终模型主要指标如下。5-fold交叉验证Accuracy为0.8641±0.0176；train内部dev阈值实验选择0.51，但应用到val.csv后准确率降到0.8753，因此最终保留0.5。事件级错误分析显示event 0和event 1的错误均为false negative，后续优化重点应放在这些事件的谣言召回。全部结果可复现。",
            body,
        )
    ]
    result_table = Table(
